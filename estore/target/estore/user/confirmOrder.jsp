...
<!--文件体开始-->
		...
		<form name="order" method="post" action="comfirmOrder">
...
<!--文件尾开始-->
		...
		<tr>
			<td class=tablebody2 valign=middle align=center width=""><c:if test="${!empty order.id}">${order.id}</c:if></td>
			<td class=tablebody1 valign=middle width="">&nbsp;&nbsp;<a href="user/productDetail?id=${book.value.book.id }" target="_blank">${book.value.book.id }</a></td>
			<td class=tablebody2 valign=middle align=center width="">价格：${book.value.book.price }</td>
			<td class=tablebody1 valign=middle align=center width="">数量：${book.value.num}</td>
			<td class=tablebody2 valign=middle align=left width="">小计：￥${book.value.num * book.value.book.price }</td>
		</tr>
        </c:forEach> 
